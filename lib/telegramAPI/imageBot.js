...
class ImageBot extends EventEmitter {
    ...
    sendImage(caption, imageUrl, chat_id) {
        let downloader = new Downloader({
            url: imageUrl,
            destPath: path.join(os.homedir(), '.lazygifter/imgs/' + misc.extratFileNameFromUrl(imageUrl))
        });
        console.log(util.format('[LAZYGIFTER][IMAGEBOT] Donwloading %s', imageUrl));
        this.downloader.on('download:success', this._sendImage.bind(this, caption, chat_id, downloader));
        this.downloader.on('download:error', this._handleDownloadImageError.bind(this));
