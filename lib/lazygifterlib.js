...
class Lazygitfter {
    ...
    _handleRequest(items, searchInformation) {
        console.log('[LAZYGIFTER] Search Info for %s time: %s total results: %s', encodeURI(this.query.params.q), searchInformation.formattedSearchTime, searchInformation.formattedTotalResults);
    ...
    _sendImages(links) {
        ...
        if (item) {
            ...
            this.imageBot.once('image:telegram:error', this._handleErrorSendTelegram.bind(this, links));
            this.imageBot.once('image:download:error', this._handleErrorDownload.bind(this, links));
