...
class Lazygitfter {
    ...
    _handleAllDownloaded(downloader) {
        if (this.options.cluster && cluster.isWorker) {
            process.send(downloader.options);
        } else {
    ...
    _cluster() {
        ...
        if (cluster.isMaster) {
            ...
            cluster.on('message', this._handleClusterMessage.bind(this));
        } else {
            this.daemon();
    ...
    _handleClusterExit(worker, code, signal) {
        ...
        APP.cpuUsage.removeDeadWorker(worker.process.pid);
        if (!worker.exitedAfterDisconnect) {
            cluster.fork();
        }
    }

    _handleClusterMessage(worker, message, handle) {
        console.log('[LAZYGIFTER][MASTER-WORKER] Message %s', message);
        let downloader = new Downloader(message);
        let sender = new Sender({
            'downloader': downloader,
            engine: this.telegramDaemon
        });
    ...
    daemon() {
        if (this.options.cluster && cluster.isWorker) {
            cluster.worker.process.on('message', this._handleWorkerMessage.bind(this));
